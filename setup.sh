...
curl -o /usr/local/bin/update-config.sh https://raw.githubusercontent.com/itsreddi/hetzner-cloud-init/master/update-config.sh
...
cat <<EOF >> /etc/crontab
*/2 * * * * root /usr/local/bin/update-config.sh --hcloud-token ${TOKEN} --whitelisted-ips ${WHITELIST_S} ${FLOATING_IPS}
EOF
